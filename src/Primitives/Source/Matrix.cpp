...
Primitives::Matrix::Matrix(Primitives::Matrix &&rhs) : width(rhs.width), height(rhs.height), data(std::move(rhs.data)) {
...
Primitives::Tuple Primitives::Matrix::operator*(Primitives::Tuple &rhs) {
  ...
  for (size_t i = 0; i < 4; ++i) {
    res[i] = this->at(0, i) * rhs.x + this->at(1, i) * rhs.y + this->at(2, i) * rhs.z + this->at(3, i) * rhs.w;
  }
